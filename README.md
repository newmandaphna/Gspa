...
5. For the day-before reminder email and class mail retries, add a `CRON_SECRET` secret (a long random value) and create one Scheduled Deployment that runs `curl -fsS -H "Authorization: Bearer $CRON_SECRET" https://gunspa.com/api/cron/run` once an hour. Each run purges expired class ID documents, retries any class mail the outbox still holds, and mails every confirmed reservation that is 12 to 25 hours away and was made a day or more ahead, once (`bookings.reminder_sent_at` records it), so with hourly runs a guest hears from us 24 to 25 hours before their slot.
