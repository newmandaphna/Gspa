...
for (const width of [1280, 390]) {
  test(`homepage classes and exact deep links at ${width}px`, async ({ page }) => {
    ...
    await expect(page.getByText("That class is no longer listed", { exact: false })).toBeVisible();
    ...
    await expect(page.getByText("That class is no longer listed", { exact: false })).toHaveCount(0);
...
test("homepage renders nothing while loading or empty, says a read failure once, and refreshes on focus", async ({ page }) => {
  let mode = "loading";
  ...
  await page.goto("/");
  const section = page.locator("#upcoming-classes");
  // Loading: no band at all, then the failure line with its retry.
  await expect(section).toHaveCount(0);
  await expect(section.getByRole("alert")).toBeVisible();
  await expect(section.getByText("The class list could not be read.")).toBeVisible();
  mode = "empty";
  await section.getByRole("button", { name: "Try again" }).click();
  // Empty: the band does not exist, so the home page never promises dates it cannot show.
  await expect(section).toHaveCount(0);
  mode = "ready";
